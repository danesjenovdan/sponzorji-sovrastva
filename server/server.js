...
razglednice.forEach((rzg) => {
  app.get(`${baseUrl}${rzg.link}`, async (req, res) => {
    try {
      ...
      $('.removeme').remove();
      $('head').append(`
        <meta property="og:url" content="https://zradirani-metelkova6.djnd.si${baseUrl}${rzg.link}">
        <meta property="og:image" content="https://zradirani-metelkova6.djnd.si${baseUrl}razglednice/${rzg.image}">
        <meta name="twitter:image" content="https://zradirani-metelkova6.djnd.si${baseUrl}razglednice/${rzg.image}">
      `);
      res.send($.html());
    } catch (err) {
      console.log(err);
